...
/**
 * Admin Portal API Call Analytics Endpoint
 * Receives and stores metrics for all API calls made from the admin portal
 * Tracks performance, errors, and usage patterns for admin operations
 */
...
async function sendToBackend(metric: any): Promise<void> {
  ...
  try {
    const response = await fetch(`${backendUrl}/admin/api-metrics`, {
      method: 'POST',
    ...
    console.error('Failed to send admin API metrics to backend:', error);
  }
}
