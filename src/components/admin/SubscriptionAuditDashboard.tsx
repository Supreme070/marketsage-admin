...
export function SubscriptionAuditDashboard() {
  ...
  const fetchData = async () => {
    try {
      const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:3006/api/v2';
      const [auditsRes, analyticsRes, issuesRes] = await Promise.all([
        fetch(`${backendUrl}/admin/subscriptions/audit`),
        fetch(`${backendUrl}/admin/subscriptions/analytics`),
        fetch(`${backendUrl}/admin/subscriptions/verify`)
      ]);
  ...
  const handleVerifySubscription = async () => {
    ...
    try {
      const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:3006/api/v2';
      const response = await fetch(`${backendUrl}/admin/subscriptions/verify`, {
        method: "POST",
