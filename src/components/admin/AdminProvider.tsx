...
export function AdminProvider({ children }: AdminProviderProps) {
  ...
  useEffect(() => {
    ...
    console.log('AdminProvider: Checking authorization for', { userEmail, userRole });

    // Check if user is authorized admin using centralized config
    const isStaff = userEmail && isAuthorizedAdmin(userEmail, userRole);

    console.log('AdminProvider: Authorization result:', isStaff);

    if (!isStaff) {
      console.warn('AdminProvider: Access denied for', userEmail);
      setAdminData(null);
      return;
    }
